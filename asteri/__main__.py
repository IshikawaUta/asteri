...
try:
    ...
    GeventWorker = None  # type: ignore
try:
    from asteri.workers.tornado import TornadoWorker
except ImportError:
    TornadoWorker = None  # type: ignore
...
def main():
    ...
    parser.add_argument(
        "app", nargs="?", help="Application path (e.g., myapp:app)")
    ...
    config_group.add_argument(
        "--print-config", action="store_true", help="Print the configuration settings."
    )

    # Network Group
    net_group = parser.add_argument_group("Network")
    net_group.add_argument("-b", "--bind", action="append",
    ...
    worker_group.add_argument(
        "-w", "--workers", type=int, default=1, help="The number of worker processes."
    )
    worker_group.add_argument(
        "-k",
        "--worker-class",
        default="sync",
        choices=["sync", "gthread", "asgi", "gevent", "tornado", "gtornado"],
        help="The type of workers to use.",
    )
    worker_group.add_argument(
        "--threads", type=int, default=1, help="The number of worker threads."
    )
    worker_group.add_argument(
        "--worker-connections", type=int, default=1000, help="Max simultaneous clients."
    )
    worker_group.add_argument(
        "--max-requests",
        type=int,
        default=0,
        help="Max requests before worker restart.",
    )
    worker_group.add_argument(
        "--max-requests-jitter", type=int, default=0, help="Jitter for max-requests."
    )
    worker_group.add_argument(
        "-t", "--timeout", type=int, default=30, help="Worker timeout in seconds."
    )
    worker_group.add_argument(
        "--graceful-timeout", type=int, default=30, help="Timeout for graceful restart."
    )
    worker_group.add_argument(
        "--keep-alive", type=int, default=2, help="Keep-alive timeout."
    )
    worker_group.add_argument(
        "--preload", action="store_true", help="Load app code before forking."
    )
    ...
    sec_group.add_argument(
        "--ssl-version", type=int, default=2, help="SSL version to use."
    )
    sec_group.add_argument("--ciphers", help="SSL Cipher suite to use.")
    sec_group.add_argument(
        "-u", "--user", help="Switch worker processes to run as this user."
    )
    sec_group.add_argument(
        "-g", "--group", help="Switch worker process to run as this group."
    )
    sec_group.add_argument(
        "-m", "--umask", type=int, default=0, help="Bit mask for file mode."
    )

    # Logging Group
    log_group = parser.add_argument_group("Logging")
    log_group.add_argument("--access-logfile", help="The Access log file.")
    log_group.add_argument("--error-logfile", "--log-file",
                           help="The Error log file.")
    ...
    log_group.add_argument(
        "--capture-output",
        action="store_true",
        help="Redirect stdout/stderr to error log.",
    )
    ...
    proc_group.add_argument(
        "--chdir", help="Change directory before loading apps.")
    proc_group.add_argument(
        "-e", "--env", action="append", help="Set environment variables."
    )
    proc_group.add_argument(
        "-n", "--name", help="Process name for setproctitle.")
    proc_group.add_argument(
        "--reload", action="store_true", help="Restart workers on code changes."
    )
    proc_group.add_argument(
        "--disable-dashboard",
        action="store_true",
    ...
    proc_group.add_argument(
        "--dirty-apps", help="Config string for routing dirty apps."
    )
    proc_group.add_argument(
        "--stash-address", help="Unix socket or host:port for StashServer."
    )
    proc_group.add_argument(
        "--statsd-host", help="StatsD host to emit metrics.")
    proc_group.add_argument(
        "--statsd-port", type=int, default=8125, help="StatsD port."
    )
    proc_group.add_argument(
        "--statsd-prefix", default="asteri", help="StatsD prefix.")
    ...
    http_group.add_argument(
        "--limit-request-field_size", type=int, default=8190)

    # HTTP/2 & HTTP/3 Group
    h2_group = parser.add_argument_group("HTTP/2 & HTTP/3 (QUIC)")
    h2_group.add_argument(
        "--http-protocols",
        default="h1",
        help="HTTP protocols to support (e.g., h1,h2,h3)",
    )
    h2_group.add_argument(
        "--http2-max-concurrent-streams", type=int, default=100)
    ...
    setup_logging(
        log_file=log_file, level=log_level, capture_output=args.capture_output
    )
    setup_access_logging(log_file=args.access_logfile,
                         log_format=args.access_logformat)

    # Auto-detect ASGI application and promote worker class
    if args.app:
        try:
            app_obj = import_app(args.app)
            import inspect

            is_asgi = False
            if callable(app_obj):
                if hasattr(app_obj, "__class__") and app_obj.__class__.__name__ in (
                    "FastAPI",
                    "Starlette",
                ):
                    is_asgi = True
            ...
            if is_asgi and args.worker_class == "sync":
                logger.info(
                    f"{Colors.GREEN}ASGI application detected. Automatically promoting worker class from 'sync' to 'asgi'.{Colors.ENDC}"
                )
                args.worker_class = "asgi"
        except Exception:
            pass
    ...
    if args.preload:
        if args.reload:
            logger.warning(
                f"{Colors.YELLOW}Both --reload and --preload are enabled. Changes to preloaded code will NOT be picked up.{Colors.ENDC}"
            )
        import_app(args.app)
    ...
    logger.info(
        f"Booting {Colors.CYAN}{args.worker_class}{Colors.ENDC} workers...")
