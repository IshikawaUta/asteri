...
def run_server(command):
    ...
    proc = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setsid)
...
def parse_ab_output(output):
    ...
    latency = re.search(r"Time per request:\s+([\d.]+)\s+\[ms\]\s+\(mean\)", output)
...
def main():
    servers = [
        ...
        {"name": "Asteri (Tornado)", "type": "WSGI", "cmd": ["python3", "-m", "asteri", "-w", "4", "-k", "tornado", "-b", "127.0.0.1:8080", WSGI_APP]},
        {"name": "Asteri (GTornado)", "type": "WSGI", "cmd": ["python3", "-m", "asteri", "-w", "4", "-k", "gtornado", "-b", "127.0.0.1:8080", WSGI_APP]},
        {"name": "Gunicorn (Sync)", "type": "WSGI", "cmd": ["gunicorn", "-w", "4", "-b", "127.0.0.1:8080", WSGI_APP]},
        
        # ASGI Servers
        {"name": "Asteri (ASGI)", "type": "ASGI", "cmd": ["python3", "-m", "asteri", "-w", "4", "-k", "asgi", "-b", "127.0.0.1:8080", ASGI_APP]},
        {"name": "Uvicorn", "type": "ASGI", "cmd": ["uvicorn", "--workers", "4", "--host", "127.0.0.1", "--port", "8080", ASGI_APP]},
    ]
    ...
    for server in servers:
        ...
        try:
            ...
            if server["cmd"][1] == "-m":
                ...
                subprocess.run([server["cmd"][0], "--version"], capture_output=True)
        except FileNotFoundError:
            print(f"Skipping {server['name']}: {server['cmd'][0]} not installed.")
            continue
        ...
        try:
            ...
            results.append({"name": server["name"], "type": server["type"], **stats})
        finally:
    ...
    print("-" * 70)
    for res in results:
        print(f"{res['name']:<20} | {res['type']:<6} | {res['rps']:<10.2f} | {res['latency']:<12.2f}")
    print("="*70 + "\n")
