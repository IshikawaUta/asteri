...
class TestStatsdIntegration(unittest.TestCase):
    ...
    def test_statsd_emissions_on_spawn_and_reap(self, mock_fork, mock_manage_workers, mock_setup_signals, mock_statsd_class):
        ...
        mock_statsd_class.assert_called_once_with("127.0.0.1", 8125, "test_asteri")
        ...
        with patch("asteri.arbiter.os.waitpid") as mock_waitpid:
            mock_waitpid.side_effect = [(12345, 0), (0, 0)] # First returns reaped worker, second returns 0
            ...
            while pid > 0:
                if pid in arbiter.workers:
                    ...
                    if arbiter.statsd:
                        ...
                        arbiter.statsd.gauge("workers.count", len(arbiter.workers))
                pid, status = mock_waitpid()
