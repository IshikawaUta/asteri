...
class TestASGIWebSocketProtocol(unittest.TestCase):
    def test_websocket_lifecycle(self):
        async def run_test():
            ...
            close_masked = bytes(b ^ mask_key[i % 4] for i, b in enumerate(close_payload))
            ...
            loop.sock_recv = AsyncMock(side_effect=[request_bytes, frame, close_frame, b""])
            ...
            self.assertTrue(handshake_resp.startswith(b"HTTP/1.1 101 Switching Protocols\r\n"))
            ...
            expected_accept = base64.b64encode(hashlib.sha1((client_key + guid).encode('utf-8')).digest())
