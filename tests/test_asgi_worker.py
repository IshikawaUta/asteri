...
class TestASGIWorker(unittest.TestCase):
    def setUp(self):
        self.worker = ASGIWorker(
            age=0, ppid=100, sockets=[], app_path="example_asgi:app", timeout=30
        )
    ...
    def test_asgi_app_execution_flow(self):
        ...
        async def run_test():
            ...
            loop.sock_recv = AsyncMock(
                side_effect=[request_bytes, b""]
            )  # first call returns request, second EOF
    ...
    def test_asgi_dashboard_disabled(self):
        ...
        async def run_test():
            ...
            args, kwargs = loop.sock_sendall.call_args
            response_sent = args[1]
            self.assertTrue(response_sent.startswith(
